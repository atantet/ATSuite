module bootstrap
  ...
  interface mbb_bivar_pval
     module procedure mbb_bivar_pval, mbb_bivar_pval_field
  end interface mbb_bivar_pval
  
  interface mbb_ci
     module procedure mbb_bivar_ci, mbb_bivar_field_ci
...
contains
  subroutine mbb_bivar_pval_field(X, statistic, sqrtNs, pval, estim, lopt, msg)
    implicit none
    ...
    if (present(msg)) then
       !$OMP PARALLEL DO PRIVATE(pval_ij, estim_ij, lopt_ij)
       do jj = 1, n
          pval(jj, jj) = 0
          estim(jj, jj) = 1
          lopt(jj, jj) = 0
  ...
  subroutine mbb_bivar_pval(x, y, statistic, sqrtNs, pval, estim, lopt, &
    ...
    real, dimension(:, :), allocatable :: x_v2, y_v2 ! like x_v, y_v but 2D
    real, dimension(1, 1) :: estim2 ! estimator as matrix
    real, dimension(:, :), allocatable :: xb, yb ! resamples of x, y
    integer :: n, n_v ! number of realisations and valid realisations
    real, dimension(sqrtNs, sqrtNs) :: set2 ! set of estimates as matrice
    ...
    if (n_v > 1) then
       ...
       if (lopt < n_v) then
          ...
          call resample_bivar(x_v, y_v, lopt, sqrtNs, xb, yb)
          ! Get statistic of resamples
          set2 = statistic(xb, yb)
  ...
  subroutine mbb_bivar_field_ci(X, statistic, sqrtNs, alpha, ci, estim, lopt, msg, ax_in)
    implicit none
    ! Dummy arguments
  ...
  end subroutine mbb_bivar_field_ci
